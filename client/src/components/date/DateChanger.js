...
export default function DateChanger(props) {
  ...
  const isDayLastInMonth = () => {
    if (
      (currentDay === 29 && currentMonth === 2 && isLeapYear()) ||
  ...
  const isDayLastInDecember = () => {
    if (currentDay === 31 && currentMonth === 12) return true
    else return false
  }
  ...
  const isDayFirstInJanuary = () => {
    if (currentDay === 1 && currentMonth === 1) return true
    else return false
  }

  const isDayFirstInMarch = () => {
    if (currentDay === 1 && currentMonth === 3) return true
    else return false
  }
  ...
  const changePreviousDay = () => {
    if (isDayFirstInJanuary()) {
      ...
      if (isLeapYear()) setCurrentDay(29)
      else setCurrentDay(28)
  ...
  return (
    <div className='center-section__top__date-changer'>
      ...
      >
        <CgChevronDoubleLeftO style={{ color: 'black' }} />
      </button>
